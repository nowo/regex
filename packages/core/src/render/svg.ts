...
.rr-active>.rr-node .rr-box,.rr-active>.rr-group,.rr-active>.rr-ccbox,.rr-active>.rr-ccitem,.rr-active>.rr-countbox{stroke-width:3;filter:drop-shadow(0 0 3px var(--rr-hl,rgba(16,185,129,.6)))}
...
.rr-charclass{fill:var(--rr-charclass-bg,#dbeafe);stroke:var(--rr-charclass-bd,#60a5fa)}
...
.rr-countbox{fill:none;stroke:var(--rr-count-bd,#f43f5e);stroke-width:1.5;stroke-dasharray:5 4}
.rr-ccbox{fill:var(--rr-cc-bg,#fef9c3);stroke:var(--rr-cc-bd,#eab308);stroke-width:1.5;cursor:pointer}
.rr-ccbox-neg{stroke:var(--rr-cc-neg-bd,#f43f5e);stroke-dasharray:5 4}
.rr-ccitem{stroke-width:1.4}
.rr-cc-literal{fill:var(--rr-literal-bg,#dbeafe);stroke:var(--rr-literal-bd,#60a5fa)}
.rr-cc-set{fill:var(--rr-charset-bg,#16a34a);stroke:var(--rr-charset-bd,#15803d)}
...
.rr-grouplabel,.rr-count,.rr-cchead{font-size:11px;fill:var(--rr-muted,#64748b)}
</style>`
...
function renderNode(node: LayoutNode): string {
    ...
    if (node.countBox) {
        ...
        parts.push(`<rect class="rr-countbox" x="${round(b.x)}" y="${round(b.y)}" width="${round(b.w)}" height="${round(b.h)}" rx="7"/>`)
    }
...
function renderTerminal(node: LayoutNode): string {
    const rx = node.cls === 'charset' ? node.height / 2 : node.cls === 'charclass' ? 4 : 6
...
function renderCharClass(node: LayoutNode): string {
    ...
    if (node.box) {
        const b = node.box
        const neg = node.negate ? ' rr-ccbox-neg' : ''
        parts.push(`<rect class="rr-ccbox${neg}" x="${round(b.x)}" y="${round(b.y)}" width="${round(b.w)}" height="${round(b.h)}" rx="8"/>`)
    }
    for (const it of node.items ?? []) {
        ...
        parts.push(
            ...
            + `<rect class="rr-ccitem rr-cc-${it.cls}" x="${round(it.x)}" y="${round(it.y)}" width="${round(it.w)}" height="${round(it.h)}" rx="${round(rx)}"/>`
